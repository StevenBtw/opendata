use bytes::Bytes;
use common::storage::{default_merge_batch, MergeOperator};
...
impl MergeOperator for GraphMergeOperator {
    fn merge_batch(&self, key: &Bytes, existing_value: Option<Bytes>, operands: &[Bytes]) -> Bytes {
        default_merge_batch(key, existing_value, operands, |key, existing, new_value| {
            ...
            match record_type {
                rt if rt == RecordType::Metadata as u8 => merge_i64_counter(existing, new_value),
