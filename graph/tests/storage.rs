...
use common::{StorageConfig, StorageBuilder, StorageSemantics};
...
// ─── 3.10 Delete node cleans up property index ──────────────────────

#[tokio::test(flavor = "multi_thread")]
async fn delete_node_cleans_property_index() {
    let db = setup().await;
    let s = store(&db);
    ...
    assert_eq!(s.find_nodes_by_property("age", &Value::Int64(30)), vec![id]);
    assert_eq!(
        s.find_nodes_by_property("name", &Value::String("Alice".into())),
        vec![id]
    );

    s.delete_node(id);

    // PropertyIndex entries should be cleaned up
    assert!(
        s.find_nodes_by_property("age", &Value::Int64(30)).is_empty(),
        "property index for age should be cleaned up on node delete"
    );
    assert!(
        s.find_nodes_by_property("name", &Value::String("Alice".into()))
            .is_empty(),
        "property index for name should be cleaned up on node delete"
    );
}

// ─── 3.11 Property overwrite updates index ──────────────────────────

#[tokio::test(flavor = "multi_thread")]
async fn set_property_overwrite_updates_index() {
    let db = setup().await;
    let s = store(&db);
    ...
    assert_eq!(
        s.find_nodes_by_property("score", &Value::Int64(10)),
        vec![id]
    );
    ...
    assert!(
        s.find_nodes_by_property("score", &Value::Int64(10))
            .is_empty(),
        "old property index entry should be removed on overwrite"
    );
    // New index entry should exist
    assert_eq!(
        s.find_nodes_by_property("score", &Value::Int64(20)),
        vec![id]
    );
}
