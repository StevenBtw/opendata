...
| 2026-04-24 | Corrected PropertyIndex staleness bound to `distinct_indexed_values - 1` (one entry always matches the current value). Linked to `Value::serialize` / `Value::deserialize` in `grafeo-common` and noted format is bincode-stable within a Grafeo minor. Closed Open Questions: edge property indexing deferred to Future Considerations (key layout is forward-compatible via a new `EdgePropertyIndex` tag); multi-graph resolved as one SlateDB instance per graph to match other OpenData subsystems. |
